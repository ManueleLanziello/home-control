...
lighting_color=(np.array([.038,.057,.085])*dimming[:,:,None]*(1-shadow_fill[:,:,None])+np.array([.20,.24,.28])*shadow_fill[:,:,None])
...
over([.018,.059,.010],grass*(.27+.08*broad+.05*grass_water))
...
sky_color=np.stack([.34+.09*cloud,.40+.10*cloud,.45+.09*cloud],axis=2)
reflection_alpha=cot_reflection*.28 if variant==1 else np.power(cot_reflection,.65)*.42+water*material_gloss*.065
# Bound absolute reflected contribution using original NIGHT luminance (lighting only).
# No night RGB or plan raster is copied into the environmental asset.
sky_luma=sky_color@np.array([.2126,.7152,.0722],dtype=np.float32)
internal=np.divide(cot_reflection,water,out=np.zeros_like(water),where=water>.001)
reflection_alpha=np.minimum(reflection_alpha,(.034+.16*night_lum)/sky_luma*(.4+.6*internal))
over(sky_color,reflection_alpha)
over([.37,.41,.43],cot_film*material_gloss*.033)
# Grass: separate, much weaker saturation/pooling profile; texture stays visible.
over([.026,.083,.075],grass_water*.11)
over([.22,.31,.33],grass_reflection*.045)
# Beige retains appropriate gloss but receives NO procedural pooling in V3.
over([.38,.41,.42],beige*(.025+.046*reflection*cloud))
...
for period in ['giorno','notte']:
    ...
    composite=Image.alpha_composite(Image.alpha_composite(original,lighting_hd),effects)
...
metadata={'version':3,'variant':variant,'seed':20260916,'viewport':[W,H],'shading_resolution':[SW,SH],'material_texture_bytes':texture.stat().st_size,'atmosphere_texture_bytes':lighting_texture.stat().st_size,'svg_bytes':(ROOT/'design/LAYER-METEO-PIOGGIA.svg').stat().st_size,'excluded_material_effect_pixels':0,'interior_atmosphere_effect_pixels':int(np.count_nonzero(la[house_hd])),'material_pixels':{n:int(np.count_nonzero(ids==idx)) for n,idx in classes.items()}, 'cotto_clusters':cot_clusters,'giardino_clusters':grass_clusters,'coverage':{
